...
function saveEdit() {
    ...
    if (newZipFile) {
        formData.append("zip_file", newZipFile);
    }
    if (newImageFile) {
...
.then(data => {
    if (data.status === "success") {
        alert("Project updated successfully! New packages installed.");
        window.location.reload();
    } else {
        alert("Error updating the project: " + data.message);
    }
})
