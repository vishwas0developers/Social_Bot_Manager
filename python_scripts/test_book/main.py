...
def load_auth_code():
    ...
    try:
        with open(AUTH_CONFIG_FILE, "r") as f:
            ...
            if not code:
                print("Auth config file is empty.")
                return None
            return code
    except FileNotFoundError:
        print(f"Auth config file not found: {AUTH_CONFIG_FILE}. Please create it or use the update feature.")
        return None
    except Exception as e:
        print(f"Error loading auth code: {e}")
        return None
...
def save_auth_code(new_code):
    ...
    try:
        ...
        print(f"Error saving auth code: {e}")
...
def download_image(image_url, folder_path):
    ...
    if not image_url or not isinstance(image_url, str):
         print(f"Skipping invalid image URL (not a string or empty): {image_url}")
         return None
    ...
    if image_url.startswith("//"):
        image_url = "https:" + image_url
         ...
         print(f"Skipping potentially relative or invalid scheme image URL: {image_url}")
         return None
    ...
    try:
        ...
        if not ext: # If no extension in path, try to get one from Content-Type or make one up
            try:
                 ...
                 print(f"HEAD request failed for {image_url}: {head_e}. Making default filename.")
        ...
        if not final_filename.lower().endswith(valid_extensions):
            print(f"Skipping non-image URL (invalid extension '{ext}'): {image_url}")
            return None
        ...
        #    print(f"Skipping non-image Content-Type '{content_type}': {image_url}")
        #    return None
        ...
        return final_filename # Return the actual saved filename
         ...
         print(f"Invalid URL (Missing Schema): {image_url}")
         return None
             ...
             if field and isinstance(field, str):
                 try:
                     ...
                     for img_tag in soup.find_all("img"):
                      ...
                      print(f"Error parsing HTML for image paths: {e}")
...
def create_zip_from_folder(folder_path):
    ...
    if not os.path.isdir(folder_path):
        print(f"Cannot create ZIP, folder not found: {folder_path}")
        return None
    ...
    with zipfile.ZipFile(master_zip_path, "w", zipfile.ZIP_DEFLATED) as mzip:
        for res in results:
            ...
            if excel_file_path and os.path.exists(excel_file_path):
                 ...
                 print(f"Master Zip: Excel file not found for {res['test_title']}: {excel_file_path}")
            ...
            if json_file_path and os.path.exists(json_file_path):
                ...
                print(f"Master Zip: JSON file not found for {res['test_title']}: {json_file_path}")
...
def cleanup_downloads():
    ...
    if os.path.exists(DEFAULT_FOLDER):
        ...
        print("No downloads folder found. Skipping cleanup.")
        ...
        for section in sections:
            ...
            for q in questions:
                ...
                if not isinstance(lang_data, dict):
                     print(f"Skipping question due to unexpected language data format: {q.get('_id')}")
                     continue
    ...
    try:
        ...
        for qid, details in answers_raw_data.items():
            if not isinstance(details, dict):
                print(f"Skipping answer for qid {qid} due to unexpected format.")
                continue
            ...
            if answer_index == 1:
                ...
                option_letter = f"Unknown({answer_index})"
...
def index_get():
    """Displays the main input form with Gemini validation checkbox."""
    auth_code = load_auth_code()
    gemini_validation_enabled = session.get("gemini_validation_enabled", False)
    if not auth_code:
        flash("Auth code is missing. Please use 'Update Auth Code' if scraping fails.", "warning")
    return render_template("index.html", gemini_validation_enabled=gemini_validation_enabled)

@app.route("/scrape", methods=["POST"])
def index_post():
    """Handles the scraping request with optional Gemini validation."""
    ...
    gemini_validation_enabled = request.form.get("gemini_validation") == "on"
    session["gemini_validation_enabled"] = gemini_validation_enabled
        ...
        try:
            ...
            for q in questions_data:
                ...
                if qid and qid in answers_mapping:
                    ...
                    answer_letter = answer_details.get("answer_letter")
                    if answer_letter and "Unknown(" in str(answer_letter):
                        idx_number = str(answer_letter).replace("Unknown(", "").replace(")", "")
                        try:
                            idx_int = int(idx_number)
                            letter_map = ["A", "B", "C", "D", "E"]
                            answer_letter = letter_map[idx_int - 1] if 1 <= idx_int <= len(letter_map) else f"Unknown({idx_number})"
                        except Exception:
                            answer_letter = f"Unknown({idx_number})"
...
def validate_answer_with_gemini(question, options, given_answer, explanation):
    gemini_api_url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent"
    gemini_api_key = "AIzaSyBm3yNZ6ksUxzlGxXkI0WTBQS3sgBotrJM"

    option_list = "\n".join([f"{chr(65 + i)}. {opt}" for i, opt in enumerate(options)])

    prompt = f"""
You are an expert MCQ checker.
...
✅ Always reply strictly in this format:
Correct Option: <letter>
Explanation: <short explanation max 2 lines>
"""
    ...
    try:
        ...
        # Broad regex to catch multiple patterns
        match = re.search(
            r"(Correct Option|The correct option is|Answer):\s*([A-H])",
            reply_text,
            re.IGNORECASE
        )
        if match:
            corrected_answer = match.group(2).upper()
            return corrected_answer, reply_text
        else:
            print("⚠️ Gemini response did not provide a clear corrected option.")
            print(f"⚠️ Gemini full reply (kept original): {reply_text}")
            return given_answer, explanation  # fallback to original

    except Exception as e:
        print(f"⚠️ Gemini validation failed: {e}")
        return given_answer, explanation  # fallback
...
def results():
    ...
    for res in results_data:
        ...
        if not test_folder_path or not os.path.isdir(test_folder_path):
             print(f"Results Page: Skipping result, test folder not found: {res.get('test_title')}")
             continue
        ...
        if not os.path.exists(individual_zip_path):
            try:
                ...
                print(f"Results Page: Created individual complete ZIP: {individual_zip_path}")
                 ...
                 print(f"Results Page: Error creating individual zip {individual_zip_path}: {e}")
