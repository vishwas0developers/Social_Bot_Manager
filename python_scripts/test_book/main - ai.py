    ...
    for url in urls:
        try:
            ...
            for q in questions_data:
                ...
                # 🌟 Preserve original explanation for images
                original_explanation_html = q["explanation"]

                # ✨ Gemini API validation step
                validated_answer, validated_explanation = validate_answer_with_gemini(
                    q["question"],
                    q["options"],
                    q["answer"],
                    original_explanation_html
                )

                if validated_answer != q["answer"]:
                    q["answer"] = validated_answer
                    q["explanation"] = validated_explanation  # short Gemini-corrected explanation
                else:
                    q["explanation"] = original_explanation_html  # keep original if already correct

                relative_image_paths_in_q = []

                for field in ["question", "explanation"]:
                    content_to_check = original_explanation_html if field == "explanation" else q.get(field)
                    if content_to_check:
                        ...
                        if field != "explanation" or validated_answer == q["answer"]:
                            q[field] = str(soup)
